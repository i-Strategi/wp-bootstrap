...
<div class="navbar-cart ml-auto dropdown <?php echo $visibility; do_action('navbar_cart_class');?>">

	<button class="navbar-btn btn <?php echo setting_navbar_btn_style(); ?> dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"  title="<?php _e('View cart','woocommerce'); ?>" type="button">
		<i class="fas fa-fw <?php echo setting_navbar_cart_icon(); ?>"></i> <?php echo WC()->cart->get_cart_total(); ?>  <span class="badge badge-pill"><?php echo sprintf(_n('%d', '%d', WC()->cart->get_cart_contents_count()), WC()->cart->get_cart_contents_count()); ?></span>
	</button>
